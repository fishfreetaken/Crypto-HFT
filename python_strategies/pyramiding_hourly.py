...
def run_pyramiding_hourly_strategy(ticker='BTC-USD', df=None, leverage_ratio=20, trailing_pct=0.03, pyramid_step_pct=0.015, target_single_trade_profit=0, fee_rate=0.0005,
    ...
    if verbose:
        print("---------------------------------------------------------")
        print(f">>> Initializing 1H-Level Pyramiding Strategy: {ticker} <<<")
        print(">>> 理论基础：市场分形几何 (Fractal Markets) <<<")
        print(f">>> 核心参数：1H周期均线 + {leverage_ratio}倍高杠杆 + {trailing_pct*100:.0f}%移动止损 + {pyramid_step_pct*100:.1f}%加仓间隔 <<<")
        print("---------------------------------------------------------")
    
    if df is None:
        try:
            # 截取近期数据进行回测
            btc = yf.download(ticker, start='2025-01-01', end='2026-03-09', interval='1h', progress=False)
    ...
    # 根据"分形的礼物"，既然大周期有效，我们将逻辑下移到纯粹的 1H 级别 (不再挂靠4H)
    ...
    for idx, row in btc.iterrows():
        ...
        if state == "WAITING" and cooldown_candles == 0:
            ...
            if _ema20 < _ema50 and _close < _ema20:
                ...
                if verbose:
                    print(f"[{idx}] 🔥 [1H信号触发] 空头排列破位做空! 价格: {_close:.2f}, 名义敞口: ${notional:.2f}")
            ...
            # 破位做多 (1H级别)
                ...
                if verbose:
                    print(f"[{idx}] 🔥 [1H信号触发] 多头排列突破做多! 价格: {_close:.2f}, 名义敞口: ${notional:.2f}")
            ...
            if direction == -1: # SHORT
                ...
                if target_single_trade_profit > 0 and unrealized >= target_single_trade_profit:
                    ...
                    if verbose:
                        print(f"[{idx}] 💥 [空头平仓] 1H宽幅震荡打穿保护 (触发价 {trailing_stop:.2f})！结转: ${total_profit:.2f}")
                    positions = []
                    state = "WAITING"
                    cooldown_candles = 12 # 1H级别降频：冷却 12 小时后重新寻找趋势机会
                    # 🔒 利润锁定检查
                ...
                if target_single_trade_profit > 0 and unrealized >= target_single_trade_profit:
                    ...
                    capital += (total_profit - fee)
                    if verbose:
                        print(f"[{idx}] 💥 [多头平仓] 1H宽幅震荡打穿保护 (触发价 {trailing_stop:.2f})！结转: ${total_profit:.2f}")
                    positions = []
                    state = "WAITING"
                    cooldown_candles = 12 # 1H级别降频：冷却 12 小时
                    # 🔒 利润锁定检查
                    if enable_profit_sweep and capital >= next_sweep_threshold:
                        excess = capital - initial_cap
                        locked = excess * sweep_keep_ratio
...
if __name__ == '__main__':
    curve, roe = run_pyramiding_hourly_strategy(ticker='BTC-USD', leverage_ratio=20, verbose=True)
    if curve is not None:
        plt.style.use('dark_background')
        plt.figure(figsize=(15, 8))
        plt.plot(curve.index, curve.values, color='#00aaff', linewidth=2)
        plt.fill_between(curve.index, 100000.0, curve.values, where=(np.array(curve.values) > 100000.0), facecolor='#00aaff', alpha=0.15)
        plt.title('Fractal Pyramiding (Pure 1H Timeframe Trend Compounding)', fontsize=14, pad=15)
        plt.xlabel('Date Time', fontsize=12)
        plt.ylabel('Strategy Equity (USD)', fontsize=12)
        plt.grid(True, linestyle=(0, (5, 10)), alpha=0.3)
        
        out_img = r'c:\Users\Administrator\Desktop\pyramiding_1H_result.png'
        plt.savefig(out_img, dpi=300, bbox_inches='tight')
        print(f"✅ 1H级别降维测算收益曲线已保存至: {out_img}")
