...
def generate_dashboard():
    ...
    lines.append(Fore.CYAN + Style.BRIGHT + f"⚡ [WebSockets实盘高频级] 1H 金字塔战法 (Binance Pro) | 当前切片: {now.strftime('%Y-%m-%d %H:%M:%S')}" + Style.RESET_ALL)
...
async def fetch_historical_binance(session, tk):
    ...
    url = f"https://data-api.binance.vision/api/v3/klines?symbol={tk}&interval=1h&limit=150"
    try:
...
async def binance_ws_stream(active_tk_list, params):
    ...
    streams = "/".join([f"{tk.lower()}@kline_1h" for tk in active_tk_list])
