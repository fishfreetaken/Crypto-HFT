...
from pyramiding_1m import run_pyramiding_1m_strategy

def evaluate_params(args):
    ticker, df, leverage_ratio, trailing_pct, pyramid_step_pct = args
    curve, roe = run_pyramiding_1m_strategy(
        ticker=ticker, 
        df=df, 
...
def run():
    ...
    trailing_pcts = [0.003, 0.005, 0.008]
    pyramid_step_pcts = [0.002, 0.004, 0.006]
    ...
    md_content = "# Top 50 加密货币 1M浮盈加仓策略 最优参数研究报告\n\n"
    md_content += "> 📈 **策略核心**: 1分钟级别 EMA20/50 顺势突破 + 分形几何金字塔浮盈大倍率加仓 + 极窄移动止损截断亏损\n"
    md_content += f"> 📅 **回测周期**: 近 {period} (包含日内各种突发极端波动)\n"
    md_content += "> ⚙️ **参数网格**: 杠杆 (10x-25x), 追踪止损点 (1%-2%), 加仓触发间隔 (0.5%-1%)\n\n"
    
    md_content += "本报告罗列了当前市场上交易量最活跃的 50 种加密货币在该无脑滚雪球策略下(1M级别)的**最强获利参数组合**与抗打击极限。\n\n"
    md_content += "---\n\n"
    ...
    md_content += "针对不同币种的1分钟微观洗盘结构（日内插针深度与单边持续力不同），即使是同样的信号引擎，也必须适配专属的加挂武器模块才能最大化生存和杀伤。\n\n"
    ...
    with open('top50_1m_report.md', 'w', encoding='utf-8') as f:
        f.write(md_content)
        
    print("Done. Saved to top50_1m_report.md")
